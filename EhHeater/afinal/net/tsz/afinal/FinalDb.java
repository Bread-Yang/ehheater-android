...
public class FinalDb {
	 ...
	 * @return 影响条数
	 */
	public int updateBySQL(String table, ContentValues values,
			String whereClause) {
		return db.update(table, values, whereClause, new  String[]{});
	}
	...
	/**
	 * REPLACE 数据库，速度要比save快
	 * 
	 * @param entity
	 */
	public void replace(Object entity) {
		checkTableExist(entity.getClass());
		exeSqlInfo(SqlBuilder.buildReplaceSql(entity));
	}
	 ...
	 *            条件为空的时候，将会更新所有的数据
	 */
	public int update(Object entity, String strWhere) {
		checkTableExist(entity.getClass());
		TableInfo table = TableInfo.get(entity.getClass());
		List<KeyValue> list = SqlBuilder.getKeyValueList(entity);
		ContentValues values = new ContentValues();
		for (int i = 0; i < list.size(); i++) {
			values.put(list.get(i).getKey(), list.get(i).getValue().toString());
		}
		int i = 0;
		try {
			i = db.update(table.getTableName(), values, strWhere,
					new String[] {});
		} catch (Exception e) {
			e.printStackTrace();
	 ...
	 * @param orderBy
	 *            排序字段
	 * @param offset 查询数量, -1 返回所有
	 */
	public <T> List<T> findAllByWhere(Class<T> clazz, String strWhere,
			String orderBy, String offset) {
		checkTableExist(clazz);
		return findAllBySql(clazz,
				SqlBuilder.getSelectSQLByWhere(clazz, strWhere) + " ORDER BY "
						+ orderBy + " LIMIT 0," + offset);
	}
	...
	/**
	 * 以transaction的方式, 执行所有SqlInfo
	 * @param li
	 */
	public void doTransactions(List<SqlInfo> li) {
		db.beginTransaction();
		try {
			for (SqlInfo sqlInfo : li) {
				db.execSQL(sqlInfo.getSql(), sqlInfo.getBindArgsAsArray());
			}
