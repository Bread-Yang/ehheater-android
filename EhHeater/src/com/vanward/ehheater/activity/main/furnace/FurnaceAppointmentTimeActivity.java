...
public class FurnaceAppointmentTimeActivity extends EhHeaterBaseActivity {
    ...
    private Dialog appointmentConflictDialog, appointmentFullDialog, appointmentExecuteTomorrowDialog;
    ...
    private void init() {
        ...
        appointmentExecuteTomorrowDialog = BaoDialogShowUtil.getInstance(this)
                .createDialogWithOneButton(R.string.appointment_execute_tomorrow,
                        BaoDialogShowUtil.DEFAULT_RESID,new OnClickListener() {
                            ...
                            public void onSuccess(String jsonString) {
                                ...
                                try {
                                    ...
                                    String responseCode = json
                                            .getString("responseCode");
                                    if ("200".equals(responseCode)) {
                                    	boolean isAllUncheck = !cb_Monday.isChecked() && !cb_Thuesday.isChecked() && !cb_Wednesday.isChecked() && !cb_Thursday.isChecked() && !cb_Friday.isChecked() && !cb_Saturday.isChecked() && !cb_Sunday.isChecked();
                                    	 // 如果小于或等于当前系统时间,则加一天时间
                                        if (isBeforeCurrentTime) {
                                            if (isAllUncheck) {  // 没有选择循环的时候,点击确认后,弹出提示框
                                                appointmentExecuteTomorrowDialog.show();
                                            } else {
                                            	finish();
                                            }
                                        } else {
