...
public class EIPatternActivity extends EhHeaterBaseActivity implements
	...
	private void setToIntelligenceMode() {
		ElectricHeaterSendCommandService.getInstance()
				.changeToIntelligenceModeWash(this);
	}
	...
	private void setToMorningWash() {
		...
		if (moringSeVo == null) {
			ElectricHeaterSendCommandService.getInstance().changeToMorningWash(
					1);
		} else {
			ElectricHeaterSendCommandService.getInstance().changeToMorningWash(
					moringSeVo.getPeople());
		}
		finish();
	}
	...
	public void setToDIY(CustomSetVo customSetVo) {
		try {
			...
			L.e(this, "自定义");
			ElectricHeaterSendCommandService.getInstance()
					.changeToZidingyiMode();
			Thread.sleep(700);
			L.e(this, "自定义 pow: " + customSetVo.getPower());
			ElectricHeaterSendCommandService.getInstance().setPower(
					customSetVo.getPower());
			Thread.sleep(700);
			L.e(this, "自定义 Tem: " + customSetVo.getTempter());
			ElectricHeaterSendCommandService.getInstance().setTempter(
