		...
		mCbShowPsw.setOnCheckedChangeListener(new OnCheckedChangeListener() {
			...
			public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
				if (isChecked) {
					...
					mEtPsw2.setInputType(InputType.TYPE_CLASS_TEXT | InputType.TYPE_TEXT_VARIATION_VISIBLE_PASSWORD);
				} else {
					mEtPsw.setInputType(InputType.TYPE_CLASS_TEXT | InputType.TYPE_TEXT_VARIATION_PASSWORD);
					mEtPsw2.setInputType(InputType.TYPE_CLASS_TEXT | InputType.TYPE_TEXT_VARIATION_PASSWORD);
				}
			}
		});
...
//				XPGConnectClient.xpgcRegister(mEtPhone.getText().toString(), mEtPsw.getText().toString());
	...
	public void OnUserRegisterResp(UserRegisterResp_t pResp, int nConnId) {
		...
		if (pResp.getResult() == 0) {
			...
			AccountService.setPendingUser(getBaseContext(), mEtPhone.getText().toString(), 
	...
	private boolean isInputValid() {
		...
		boolean phoneNotEmpty = !TextUtils.isEmpty(mEtPhone.getText().toString());
		boolean lengthGt6 = mEtPsw.getText().toString().length() >= 6;
		boolean pswMatch = mEtPsw.getText().toString().equals(mEtPsw2.getText().toString());
